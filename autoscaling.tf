...
resource "aws_autoscaling_group" "rdemo_asg" {
  name     = "rdemo_asg"
  min_size = 1
  max_size = 2
  ...
  load_balancers            = ["${aws_elb.rdemo_elb.name}"]
...
resource "aws_autoscaling_policy" "AddInstancesPolicy" {
  ...
  cooldown               = 60
  autoscaling_group_name = aws_autoscaling_group.rdemo_asg.name
}
...
resource "aws_cloudwatch_metric_alarm" "avg_cpu_ge_80" {
  ...
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.rdemo_asg.name
  }
